...
public class Server {
	 ...
	 */
	private boolean generateSubkey(){
		ScannerInfo sInfo = null;
		AES a = new AES();
		...
		try {
				...
				if(sInfo!=null)
					...
					System.out.println("数据库中不存在该读写器信息！");
					return false;
				}
		} catch (Exception e) {
			// TODO Auto-generated catch block
	...
	public void addScannerInfo(String RID,String rootKey){
		ScannerInfo sInfo = new ScannerInfo();
		AES a = new AES();
		a.setKeyValue(keyForScannerInfo);
		try {
			sInfo.setSerialNumber(RID);
			sInfo.setRootKey(a.encrypt(rootKey));
			Database.OpScannerInfo.add(sInfo);
		} catch (Exception e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
	...
	public static void main(String[] args) {
		Server s = new Server();
		//s.addUserInfo("20125193","Xu", "F", "1000");
		s.addScannerInfo("01070504", "first");
