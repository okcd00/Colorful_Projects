...
/**
 * This class is responsible for the data forwarding and receiving.
 */
...
public class Router extends Thread{
	...
	public Router() {
		...
		routingTablePanel.add(new JLabel("Router 3"));
		...
		routingTablePanel.add(new JLabel("1"));
		routingTablePanel.add(new JLabel("Router 1"));
		...
		try {
			...
			if (nextHop.equals("-")) {
				Socket socket = new Socket(InetAddress.getByName(new Config().host2IP),
						new Config().host2Port);
				OutputStream outputStream = socket.getOutputStream();
				InputStream inputStream = socket.getInputStream();
				String strNetwordData = serverReceiveDataSender+"."+serverReceiveDataMessage;
			...
			else if(nextHop.equals("Router 3")){
				Socket socket = new Socket(InetAddress.getByName(new Config().host3IP),
						new Config().router3Port);
				OutputStream outputStream = socket.getOutputStream();
				InputStream inputStream = socket.getInputStream();
				String strNetwordData = receiveStringData;
				byte[] sendByteData = strNetwordData.getBytes();
	...
	public static void server() {
		try {
			ServerSocket severSocket = new ServerSocket(new Config().router2Port);
			while (true) {
