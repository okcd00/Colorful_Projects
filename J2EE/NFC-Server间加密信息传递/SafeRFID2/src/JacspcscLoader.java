...
public class JacspcscLoader {
	...
	private static int hContext, cchReaders, hCard, dwActiveProtocols,
			cbRecvLength;
	private static int cbAtrLen, dwState, dwProtocol;
	private static int cchReaderLen, lpBytesReturned;
	private static int rsUserData, rsRdrEventState, rsATRLength,
			rsRdrCurrState;
	...
	public int jSCardEstablishContext(int dwscope, int pvReserved1,
			int pvReserved2, int[] phContext) {
	...
	public int jSCardListReaders(int[] phContext, int mszGroups,
			byte[] szReaders, int[] pcchReaders) {
	...
	public int jSCardConnect(int[] phContext, String myReader, int dwShareMode,
		...
		ret = SCardConnect(phContext[0], szReader, dwShareMode,
				dwPreferredProtocols, hCard, dwActiveProtocols);
		phCard[0] = hCard;
		pdwActiveProtocols[0] = dwActiveProtocols;
		return ret;
	};

	public int jSCardReconnect(int[] phCard, int dwShareMode,
			int dwPreferredProtocols, int dwInitialization,
			int[] pdwActiveProtocols) {
		ret = SCardReconnect(phCard[0], dwShareMode, dwPreferredProtocols,
				dwInitialization, dwActiveProtocols);
		pdwActiveProtocols[0] = dwActiveProtocols;
		return ret;
	};
	...
	public int jSCardTransmit(int[] phCard,
			ACSModule.SCARD_IO_REQUEST pioSendPci, byte[] pbSendBuffer,
			int cbSendLength, ACSModule.SCARD_IO_REQUEST pioRecvPci,
			byte[] pbRecvBuffer, int[] pcbRecvLength)

	{
		try {
			cbRecvLength = pcbRecvLength[0];
			ret = SCardTransmit(phCard[0], pioSendPci, pbSendBuffer,
					cbSendLength, pioRecvPci, pbRecvBuffer, cbRecvLength);
	...
	public int jSCardState(int[] phCard, int[] pdwState, int[] pdwProtocol,
			byte[] pbATR, int[] pcbAtrLen) {
	...
	public int jSCardGetStatusChange(int[] phContext, int dwTimeout,
			ACSModule.SCARD_READERSTATE rgReaderStates, int cReaders) {
		...
		// private static int rsUserData, rsRdrCurrState, rsRdrEventState,
		// rsATRLength;

		ret = SCardGetStatusChange(phContext[0], dwTimeout, cReaders, RdrName, // ----------
				rsUserData, //
	...
	public int jSCardStatus(int[] phCard, byte[] mszReaderNames,
			int[] pcchReaderLen, int[] pdwState, int[] pdwProtocol,
			byte[] pbATR, int[] pcbAtrLen) {
		...
		try {
			ret = SCardStatus(phCard[0], mszReaderNames, cchReaderLen, dwState,
					dwProtocol, pbATR, cbAtrLen);
	...
	public int jSCardControl(int[] phCard, int dwControlCode,
			byte[] lpInBuffer, int nInBufferSize, byte[] lpOutBuffer,
			int[] nOutBufferSize, int[] plpBytesReturned) {

		cbRecvLength = nOutBufferSize[0];
		lpBytesReturned = plpBytesReturned[0];
		ret = SCardControl(phCard[0], dwControlCode, lpInBuffer, nInBufferSize,
				lpOutBuffer, cbRecvLength, lpBytesReturned);
	...
	private native int SCardEstablishContext(int dwscope, int pvReserved1,
			int pvReserved2, int phContext);

	private native int SCardReleaseContext(int hContext);

	private native int SCardListReaders(int phContext, int mszGroups,
			byte[] szReaders, int pcchReaders);

	private native int SCardConnect(int hContext, byte[] szReaders,
			int dwShareMode, int dwPreferredProtocols, int phCard,
			int pdwActiveProtocols);

	private native int SCardReconnect(int hContext, int dwShareMode,
			int dwPreferredProtocols, int dwInitialization,
			int pdwActiveProtocols);
	...
	private native int SCardState(int hCard, int pdwState, int pdwProtocol,
			byte[] pbATR, int pcbAtrLen);

	private native int SCardGetStatusChange(int hContext, int dwTimeout,
			int cReaders,

			// SCARD_READERSTATE structure variables
			byte[] RdrName, int UserData, int RdrCurrState, int RdrEventState,
			int ATRLength, byte[] pbATR);

	private native int SCardStatus(int hCard, byte[] mszReaderNames,
			int pcchReaderLen, int pdwState, int pdwProtocol, byte[] pbATR,
			int pcbAtrLen);
	...
	private native int SCardControl(int hCard, int dwControlCode,
			byte[] lpInBuffer, int nInBufferSize, byte[] lpOutBuffer,
