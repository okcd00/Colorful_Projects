...
public class Host extends Thread implements ActionListener {
	...
	public static void server() {
		...
		try {
			ServerSocket severSocket = new ServerSocket(new Config().host1Port);
			while (true) {
				...
				if (destination != "") {
					try {
						...
						networkData = new NetworkData(userData.destination,
								"Host 1", userData.message);
						strNetwordData = networkData.destination + "."
								+ networkData.sender + "."
								+ networkData.message;// Data string transported
														// between two nodes.
						Socket socket = new Socket(
								InetAddress.getByName(new Config().host1IP), new Config().router1Port);
